...
export function useImagePaste(enabled: boolean, upload: (files: File[]) => void, allowDialog = false) {
  useEffect(() => {
    if (!enabled) return
    const paste = (event: ClipboardEvent) => {
      if (event.defaultPrevented || !event.clipboardData) return
      const target = event.target instanceof Element ? event.target : null
      if (!allowDialog && target?.closest('[role="dialog"], [role="alertdialog"]')) return
