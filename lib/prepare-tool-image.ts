...
export async function prepareToolImage(file: File, policy: UploadPolicy, notice?: (text: string) => void): Promise<File> {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error('无法读取这张图片，请尝试导出为 JPG 或 PNG 后上传'))
      image.src = url
    })
    const width = image.naturalWidth, height = image.naturalHeight
    if (!width || !height) throw new Error('图片尺寸无效，请重新选择')
    if (file.size <= policy.maxBytes && width * height <= policy.maxPixels && Math.max(width, height) <= policy.maxEdge) return file
    notice?.('图片较大，正在自动优化…')
