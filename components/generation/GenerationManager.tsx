...
const missingLabel = (value: string) => ({ 'text_model.base_url': '文字 API 地址', 'text_model.model': '文字模型', 'image_model.base_url': '图片 API 地址', 'image_model.model': '图片模型', CONTENT_TEXT_API_KEY: '文字服务密钥', CONTENT_IMAGE_API_KEY: '图片服务密钥', source_ids: '采集来源' }[value] || value)
  ...
  return <div className={styles.page}>
    ...
    <Drawer title={selectedTask ? `配置 · ${selectedTask.name}` : '新建生成任务'} open={!!editing} onClose={() => setEditing(null)} width={640} destroyOnClose>
      <Form className={styles.form} form={form} layout="vertical" onFinish={saveTask}>
        ...
        {(['textModel', 'imageModel'] as const).map(key => <fieldset key={key}><legend>{key === 'textModel' ? '文字模型' : '图片模型'}</legend>
          {key === 'textModel' && <Form.Item name={['config', key, 'provider']} label="生成方式"><Radio.Group options={[{ value: 'codex', label: 'Codex CLI' }, { value: 'openai_compatible', label: '兼容 API' }]} /></Form.Item>}
          {key !== 'textModel' || textProvider !== 'codex' ? <><Form.Item name={['config', key, 'baseUrl']} label="兼容 API 地址（包含 /v1）"><Input type="url" placeholder="https://服务地址/v1" /></Form.Item>
            <Form.Item name={['config', key, 'credentialRef']} label="服务端密钥环境变量名" extra="只填写变量名，密钥在服务器配置。"><Input autoComplete="off" /></Form.Item></> : <p className={styles.muted}>使用服务账户已登录的 Codex CLI。服务器需安装 Codex，并完成登录。</p>}
          <Form.Item name={['config', key, 'model']} label="模型名称" extra={key === 'textModel' && textProvider === 'codex' ? '留空使用 Codex 默认模型。' : undefined}><Input /></Form.Item>
          <Form.Item name={['config', key, 'timeout']} label="请求超时（秒）"><InputNumber min={30} max={600} /></Form.Item>
          {key === 'imageModel' && <><Form.Item name={['config', key, 'size']} label="图片尺寸"><Input /></Form.Item><Form.Item name={['config', key, 'responseFormat']} label="图片返回格式"><Radio.Group options={[{ value: 'auto', label: '服务商默认' }, { value: 'b64_json', label: 'Base64' }]} /></Form.Item></>}
        </fieldset>)}
