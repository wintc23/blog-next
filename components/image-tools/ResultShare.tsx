...
export default function ResultShare({ token, initialData = null }: { token: string; initialData?: ImageShare | null }) {
  ...
  return <div className={styles.shell}>{error ? <div role="alert" className={styles.error}>{error}</div> : !data ? <Spin /> : <><div className={styles.hero}><h1>{data.name} · 创作分享</h1><div className={styles.resultToolbar}><LikeButton target="image-share" id={token} /><Link href={`/tools/${data.toolSlug}`}>我也来试试</Link></div></div><div className={styles.sharedResults}>{data.outputs.map((asset, index) => <article className={styles.panel} key={asset.id}>
